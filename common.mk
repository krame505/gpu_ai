...
CXX        := g++
CC         := gcc
LINK       := nvcc --compiler-options -fPIC
...
CXXFLAGS  := $(CXXWARN_FLAGS) $(CXX_ARCH_FLAGS) -std=gnu++11
...
ifeq ($(dbg),1)
	NVCCFLAGS   += -D_DEBUG -g
	CXXFLAGS    += -D_DEBUG -ggdb -g3
	CFLAGS      += -D_DEBUG -ggdb -g3
	BINSUBDIR   := debug
	LIBSUFFIX   := D
else 
	COMMONFLAGS += -O3 
	BINSUBDIR   := release
	LIBSUFFIX   := 
	NVCCFLAGS   += -DNDEBUG --compiler-options -fno-strict-aliasing
	CXXFLAGS    += -DNDEBUG -fno-strict-aliasing
...
$(TARGET): makedirectories $(OBJS) $(CUBINS) $(PTXBINS) Makefile common.mk
	$(VERBOSE)$(LINKLINE)
...
tidy :
	$(VERBOSE)find $(SRCDIR) | egrep "#" | xargs rm -f
	$(VERBOSE)find $(SRCDIR) | egrep "\~" | xargs rm -f
